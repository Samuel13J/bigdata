...
## 你如何理解数仓 ##
&emsp;&emsp;我觉得数仓主要是面向主题去构建的，通过对业务需求的分析，划分出主题域和业务过程，本质上其实就是OLAP，是面向分析的，所以它本身是不生产数据的，
这也是区别于传统OLTP数据库最大的不同，传统的OLTP数据库，更多的是用来存储业务数据。数仓是通过接入业务系统来进行数据的采集，经过ETL转换、
过滤、清洗，分层建模，最后将分析结果再交给各个外部系统去使用。
...
## 如何划分主题域 ##
1. 按照系统划分:  
   &emsp;一个业务系统抽象出一个主题域,业务系统有几种,就划分几类
2. 按照业务过程划分:  
   &emsp;一个业务过程抽象出一个主题域,比如业务系统中有商品、交易、物流等
3. 按照需求方划分:  
   &emsp;按需求方去进行划分对应的主题域
4. 按照部门划分:
5. 按照功能或应用划分
6. 按照行业经验划分
...
## spark调优 ##
1. 参数调优：  
   &emsp;Executor core，Executor memory
2. 高性能算子调优：  
   1. &emsp;mapPartition  
   2. &emsp;foreachPartition  
   3. &emsp;filter后使用coalsesce减少分区数量  
   4. &emsp;persist / cache 对数据持久化  
   5. &emsp;reduceByKey和aggrateByKey取代groupByKey  
   6. &emsp;repartition解决sparksql低并行度的性能问题<br>
3. 开发调优：  
    1. 广播变量(broadcast) 结合业务说明  
    2. 尽量避免复用同一个RDD，如果需要，则进行cache持久化  
    3. 尽量避免shuffle类算子(reduceByKey, join, distinct, repartition)  
    4. 使用kryo优化序列化性能
